...
cross_validation_mPLSDA <- function(X, y, method, options, n_trials) {
  X <- as.matrix(X)
  y <- y
  ...
  for (trial in 1:n_trials) {
    ...
    for (fname in fold_names) {
      ...
      y_pred[indices] <- predict(model, as.matrix(X_pred[, real_features, drop = FALSE]))
    }
    ...
    if (n_trials_p > 0) {
      ...
      for (trial_p in 1:n_trials_p) {
        if (options$paired) { # create permuted y, flip all pairs in fold
          for (fname in fold_names) {
            indices <- folds[[fname]]
            #tmp_rn <- runif(length(indices)/2)
            #flip_pairs <- which(tmp_rn > 0.5)
...
#perform cross validation and get AUCROC plots out
validate_plusAUCROC <- function(X, y, method, options) {
  # ----------------- INITIAL PROCESSING ----------------- #
  ...
  for (fname in fold_names) {
    ...
    try_out <- try(y_pred[indices] <- predict(model,
                   ...
                   silent = T)
    if (is(try_out, "try-error")) {
      # No model could be built, provide a model with two latent variables
      # (even if this is not significant)
      opts_model <- list(n_LV = 2)
      model <- train(as.matrix(X_train[, features, drop = FALSE]), y_train, opts_model)
      y_pred[indices] <- predict(model,
...
validate_repeat_plusAUCROC <- function(X, y, method, options, n_trials) {
  ...
  for (trial in 1:n_trials) {
    message(paste("validate_repeat: trial", trial, "/", n_trials))
    val_scores <- validate_plusAUCROC(X, as.numeric(y), method, options)
    return_vals[[trial]] <- val_scores$cv_y
  }
  return(return_vals)
}
